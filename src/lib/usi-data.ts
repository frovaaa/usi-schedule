...
export const formatCourses = (
  ...
  const isSpringSemester = currentMonth >= 2 && currentMonth <= 7;
  const semesterYear = isSpringSemester ? currentYear : currentYear + 1;
  const semesterCode = isSpringSemester ? 'SP' : 'SA';
  ...
  return courses
    .filter((course) => {
      ...
      return (
        course.semester_academic_year.includes(semesterYear.toString()) &&
        course.semester_academic_year.includes(semesterCode)
      );
    })
    .map((course) => {
      if (
